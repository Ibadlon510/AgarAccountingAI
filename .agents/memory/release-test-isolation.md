...
**Why:** LedgerFlow's transition-audit records are intentionally append-only, so ordinary relational cleanup can leave records behind or fail. Retained HTTP keep-alive connections can also prevent a test process from exiting even after all assertions pass. Concurrent validations reset the same schema and can remove tables from an otherwise passing run.

**How to apply:** Hold the workspace-wide API CI lock across schema reset, migrations, and tests so concurrent validations cannot drop each other's tables. Provision only a database whose name is explicitly recognized as a test database, reset its schema before migrations, keep API tests serial, and close active server connections and pools during teardown. Do not weaken audit immutability merely to make cleanup succeed.
