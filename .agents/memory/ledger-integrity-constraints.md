...
**How to apply:** When changing a named check constraint or adding a relationship that must not cross a LedgerFlow client boundary, reconcile it idempotently in the integrity installer. Keep the installer in every database-push path, and test both a fresh database and one retaining the previous constraint.